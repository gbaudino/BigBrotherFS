...
static void write_dir_entry(fat_file parent, fat_file file) {
    ...
    if (chunk_size <= file->pos_in_parent * entry_size) {
        ...
        // A Arreglar: Agregamos un cluster cuando se llena el directorio. 
                    // Funciona pero al volver a montar se rompe
        /*
        //Buscar el siguiente cluster libre
        u32 cluster_to_add = fat_table_get_next_free_cluster(parent->table);
        //Apuntar al final con ese cluster nuevo
        fat_table_set_next_cluster(parent->table, cluster_to_add, FAT_CLUSTER_END_OF_CHAIN_MAX);
        //Linkear clusters
        u32 cur_cluster = parent->start_cluster;
        while (le32_to_cpu(((const le32 *)parent->table->fat_map)[cur_cluster]) != FAT_CLUSTER_END_OF_CHAIN_MAX) {
            cur_cluster = (u32)le32_to_cpu(((const le32 *)parent->table->fat_map)[cur_cluster]);
            DEBUG("cur_cluster: %d", cur_cluster);
        }
        fat_table_set_next_cluster(parent->table, cur_cluster, cluster_to_add);
        */
       return;
    }
    off_t parent_offset =
...
void fat_file_init_dir(fat_file file) {
    size_t entry_size = sizeof(struct fat_dir_entry_s);
    off_t dir_offset =
        fat_table_cluster_offset(file->table, file->start_cluster);
    u32 *buf = alloca(entry_size);
    *buf= 0;
    pwrite(file->table->fd, buf, entry_size, dir_offset);
}

void fat_file_free_clusters(fat_file file) {
    u32 next_cluster = 0, cur_cluster = file->start_cluster;
    while (fat_table_cluster_is_valid(cur_cluster)) {
        next_cluster = fat_table_get_next_cluster(file->table, cur_cluster);
        pwrite(file->table->fd, &cur_cluster, sizeof(u32),
               fat_table_cluster_offset(file->table, cur_cluster));
